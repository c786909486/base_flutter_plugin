...
class NameUtils {
    ...
    // 大驼峰转下划线命名
    static toSnakeCase(pascalCase: string): string {
        return pascalCase
            .replace(/([A-Z])/g, '_$1')
            .toLowerCase()
            .replace(/^_/, '');
    ...
    // 验证是否为有效的Dart标识符
    static isValidDartIdentifier(name: string): boolean {
...
class TemplateGenerator {
    // 生成 BaseViewModel 模板
    static generateBaseViewModel(viewModelName: string, pageName: string): string {
...
`;
    ...
    static generateBaseListViewModel(viewModelName: string, pageName: string): string {
        const className = NameUtils.toPascalCase(viewModelName);
        return `import 'package:base_flutter/base_flutter.dart';
...
`;
    ...
    // 生成 basePage 模板
    static generateBasePage(viewModelName: string, pageName: string): string {
...
`;
    ...
    // 生成 listPage 模板
    static generateListPage(viewModelName: string, pageName: string): string {
        const widgetName = NameUtils.toPascalCase(pageName);
        const viewModelClassName = NameUtils.toPascalCase(viewModelName);
        return `import 'package:base_flutter/base_flutter.dart';
...
`;
    ...
    // 生成 refreshPage 模板
    static generateRefreshPage(viewModelName: string, pageName: string): string {
        const widgetName = NameUtils.toPascalCase(pageName);
        const viewModelClassName = NameUtils.toPascalCase(viewModelName);
        return `import 'package:base_flutter/base_flutter.dart';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 注册命令
    let disposable = vscode.commands.registerCommand('base-flutter.newPage', async (uri?: vscode.Uri) => {
        // 获取当前工作区
        ...
        if (fs.existsSync(viewModelPath)) {
            const overwrite = await vscode.window.showWarningMessage(
                `ViewModel 文件已存在: ${viewModelFileName}`,
                '覆盖',
                '取消'
            );
            if (overwrite !== '覆盖') {
